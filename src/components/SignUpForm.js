...
const passwordStrengthError =
  'Your password must be at least 8 characters long, contain at least one uppercase letter, one number, and one special character. Please try again';
...
const SignUpForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (password !== rePassword) {
      setReError('Please enter correct password');
      valid = false;
    } else {
      setReError('');
    }
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#172554] bg-[url('/src/assets/banner.png')] bg-cover bg-center">
      <div className="bg-white mx-4 sm:mx-6 my-6 rounded-2xl shadow-lg p-8 w-full max-w-md flex flex-col gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-gray-500">Welcome! Enter your credentials to create your account</p>
        </div>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 ${reError ? 'border-red-400 focus:ring-red-200' : 'border-gray-200 focus:ring-blue-200'}`}
          ...
          {reError && <p className="text-xs text-red-500 ml-1">{reError}</p>}
